...
namespace BioMetrixCore
{
    public class ZkemClient : IZKEM
    {
        ...
        {
            Console.Out.WriteLine("El código  se acaba de registrar");
        }

        private void ObjCZKEM_OnEnrollFinger(int EnrollNumber, int FingerIndex, int ActionResult, int TemplateLength)
        {
            Console.Out.WriteLine("El código " + EnrollNumber + " se acaba de registrar");
        }
        ...
        public bool SetDeviceTime2(int dwMachineNumber, DateTime date)
        {
            return objCZKEM.SetDeviceTime2(dwMachineNumber, date.Year, date.Month, date.Day, date.Hour, date.Minute, date.Second);
        }
